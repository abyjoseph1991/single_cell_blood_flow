...
frameAverageMode=1; % Recommended: Mode #1
...
for fileNo=1:nFiles
    ...
    if isempty(vesselAngle)
        ...
        if nFramesCartesian>=minFramesforSTD
            cartesianFrame=mean(cartesianVideoData(:,:,2:end),3); % ignoring first frame due to unknown DeMotion bug in first frame of final registered video
